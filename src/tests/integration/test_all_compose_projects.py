...
def test_status_shows_only_same_project_by_default(
    run_rcb_command, secondary_compose_up
):
...
def test_backup_only_same_project_by_default(
    run_rcb_command, secondary_compose_up, secondary_web_container, create_test_data
):
...
def test_backup_all_compose_projects_volumes(
    backup_container_with_multi_project,
    secondary_web_container,
    create_test_data,
    project_root,
):
    ...
    create_test_data(
        "test_data/secondary_web/multi_project_test.txt", test_content_bind
    )
    time.sleep(2)
...
def test_backup_all_compose_projects_databases(
    ...
    assert (
        "Backing up databases" in backup_output or "database" in backup_output.lower()
    )
...
def test_all_projects_after_multiple_backups(
    backup_container_with_multi_project, secondary_web_container, create_test_data
):
    ...
    snapshot_lines = [
        line
        for line in snapshots_output.split("\n")
...
def test_include_all_projects_with_excluded_services(
    backup_container_with_multi_project,
):
...
def test_project_name_in_backup_path_with_all_projects(
    backup_container_with_multi_project, create_test_data
):
    """Test that when INCLUDE_ALL_COMPOSE_PROJECTS is enabled, project names are included in paths"""
    # Create test data
    create_test_data(
        "test_data/secondary_web/project_name_test.txt", "Testing project names"
    )
    time.sleep(2)
